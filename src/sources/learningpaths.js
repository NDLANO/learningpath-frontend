...
const fetchPathStep = fetchAuthorized("/learningpath-api/v2/learningpaths/:pathId/learningsteps/:stepId");

const fetchMyPaths = fetchAuthorized("/learningpath-api/v2/learningpaths/mine/");

const fetchPathsWithStatus = fetchAuthorized("/learningpath-api/v2/learningpaths/status/:learningPathStatus");

const fetchPathTags = (_, locale) =>
  fetch(`${baseUrl}/tags/?language=${locale}&fallback=true`).then(resolveJsonOrRejectWithError);

const fetchPathContributors = () => fetch(`${baseUrl}/contributors/`).then(resolveJsonOrRejectWithError);
...
const createPath = (props, body) =>
  postLearningPath(props, body).then((lpath) =>
    Promise.all(map(body.learningsteps, (step) => postLearningPathStep({ pathId: lpath.id }, step))).then((steps) =>
      Object.assign({}, lpath, {
...
const updateStep = ({ pathId, stepId }, body) => patchLearningPathStep({ pathId, stepId }, body);
...
const deleteStep = ({ pathId, stepId }) => deleteLearningPathStep({ pathId, stepId });
...
const updateStatus = ({ pathId }, body) => putLearningPathStatus({ pathId }, body);

export const activateDeletedPath = ({ pathId, status }) => putLearningPathStatus({ pathId }, { status });

const updateSeqNo = ({ pathId, stepId }, body) => putSequenceNumber({ pathId, stepId }, body);
