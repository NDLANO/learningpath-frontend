...
export default class LearningPathStepFields extends React.Component {

  componentWillReceiveProps() {
    const { title, oembedPreview, lang } = this.props;
    const embedContent = oembedContentI18N({ embedUrl: oembedPreview }, lang);
    if ((!title.input.value || title.input.value.length === 0) && (embedContent && embedContent.title)) {
      title.input.onChange(embedContent.title);
    }
  }
