...
export function validateOembed(
  url,
  lang,
  embedType = 'omebed',
  fieldName = 'url',
  msgKey = 'validation.oembed',
) {
  if (!url || url.length === 0) {
    return dispatch =>
      new Promise(resolve => {
  ...
  if (embedType === 'lti') {
    return dispatch =>
      new Promise(resolve => {
        ...
        dispatch(
          setOembedPreview(
            Object.assign({}, oembed, { url, embedType, language: lang }),
          ),
        );
        resolve();
      });
    ...
    new Promise((resolve, reject) =>
      fetchOembedUrl({ url })
        .then(oembed => {
          ...
          const currentOembedTitle = get(
            state,
            'oembedPreview.oembedContent[0].title',
          );
          const currentFormTitle = get(
            state,
            'form.learning-path-step.values.title',
          );
          ...
          if (
            oembed.title &&
            (!currentFormTitle || currentOembedTitle === currentFormTitle)
          ) {
