...
test("actions/createLearningPathStep", (done) => {
  ...
  const postPathStepApi = nock("http://ndla-api", {
    ...
    .post(`/learningpath-api/v2/learningpaths/${pathId}/learningsteps/`, learningStep)
...
test("actions/createLearningPathStep access denied", (done) => {
  ...
  store
    ...
    .then(() => {
      expect(store.getActions()).toEqual([
        applicationError(
          payload403invalid(`http://ndla-api/learningpath-api/v1/learningpaths/${pathId}/learningsteps/`),
        ),
        {
          payload: {
