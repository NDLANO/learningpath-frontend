...
test("reducers/learningPath", () => {
  ...
  expect(reducer({ id: "abc" }, { type: "DO_NOT_SET_LEARNING_PATH", payload })).toEqual({ id: "abc" });

  expect(reducer({ id: "abc" }, { type: "SET_LEARNING_PATH", payload: new Error("fail"), error: true })).toEqual({
    id: "abc",
  });
});
