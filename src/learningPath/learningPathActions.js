...
export const setLearningPathsStatus = createAction(
  'UPDATE_LEARNING_PATHS_STATUS',
);
export const removeLearningPath = createAction('REMOVE_LEARNING_PATH');
export const setLearningPathStatus = createAction(
  'UPDATE_LEARNING_PATH_STATUS',
);
export const setLearningPathRevision = createAction(
...
export function updateLearningPath(
  pathId,
  learningPath,
  redirectUrl = `/learningpaths/${pathId}`,
) {
  return dispatch =>
    new Promise((resolve, reject) =>
...
export function activateDeletedLearningPath(pathId, status) {
  return dispatch =>
    activateDeletedPath({ pathId, status }).then(() =>
...
export function copyLearningPath(learningPath, locale) {
  const copiedTitle = polyglot
    .t('copyLearningPath.copy')
    .concat(learningPath.title.toString());
          ...
          dispatch(
            routerActions.push({
              pathname: lpath.learningsteps[0]
                ? `/learningpaths/${lpath.id}/step/${
                    lpath.learningsteps[0].id
                  }/`
