...
test("actions/fetchImage with id", (done) => {
  const apiMock = nock("http://ndla-api").get(`/image-api/v2/images/${imageId}`).reply(200, { id: imageId });
...
test("actions/fetchImage with id access denied", (done) => {
  const apiMock = nock("http://ndla-api").get(`/image-api/v2/images/${imageId}`).reply(403, { message: "Invalid" });
